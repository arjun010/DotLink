...
function refreshData(){
  ...
  firstTime=1;
  for(var i=0;i<linkedByIndex.length;i++){
    linkedByIndex[i]=0;
  }
...
d3.selectAll(".pathviewlink")
  ...
  .style("opacity",function(d){
    var allTypesInCurrentEdge = getAllAllianceTypesBetween(d.source.name,d.target.name);
    for(var i=0;i<relationshipTypesToShow.length;i++){
      if(objectIsInList(relationshipTypesToShow[i],allTypesInCurrentEdge)==1){
        nodesToShow.push(d.source.name);
        nodesToShow.push(d.target.name);
        return 0.8;
      }
    }
    if(showCB==1){
      if(edgeHasCrossBorderAlliance(d.source.name,d.target.name)==1){
        nodesToShow.push(d.source.name);
        nodesToShow.push(d.target.name);
        return 0.8;
      }
    ...
    if(showJV==1){
      if(edgeHasJointVenture(d.source.name,d.target.name)==1){
        nodesToShow.push(d.source.name);
        nodesToShow.push(d.target.name);
        return 0.8;
      }
  ...
  d3.selectAll(".pathviewnode")
    .transition().duration(300)
    .style("opacity",function(d){
      if(objectIsInList(d.name,nodesToShow)==1){
        return 0.8;
      }else{
        return 0;
      }
...
function drawScatternetView(mode){
    ...
    if($("#scatternetscale").val()=="linear"){
      ...
      y=d3.scale.log();
        ...
        x.domain(d3.extent(globalNodes, function (d) {
    ...
    // this does the same as for the y axis but maps from the allianceCount variable to the height to 0. 
        ...
        y.domain(d3.extent(globalNodes, function (d) {
    ...
    if(globalLinks.length>1){
      scatternetLinks =computeScatternetLinks();
    }
...
/*
function scatternetZoom() {
      svg.select("g.scatternet_y_axis").call(yAxis);
      svg.select("g.scatternet_x_axis").call(xAxis);
      svg.selectAll(".scatternetnode")      
          .attr("transform", function(d, i) {
              var xVal,yVal;
              if(document.getElementById("xaxis").value=="allianceCount"){
                xVal = d.allianceCount;
        }else if(document.getElementById("xaxis").value=="highPrice"){
                xVal = d.highPrice;
        }else if(document.getElementById("xaxis").value=="companySize"){
                xVal = d.companySize;
        ...
        if(document.getElementById("yaxis").value=="allianceCount"){
                yVal = d.allianceCount;
        }else if(document.getElementById("yaxis").value=="highPrice"){
                yVal = d.highPrice;
        }else if(document.getElementById("yaxis").value=="companySize"){
                yVal = d.companySize;
        }
        console.log("translate("+x(xVal)+","+y(yVal)+")");
...
if(globalNodes.length>1){
  segmentViewLinks =computeSegmentViewLinks();
}
