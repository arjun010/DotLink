...
function linkIsInList(link,exisitingLinks){
  for(var i=0;i<exisitingLinks.length;i++){
    if(typeof(exisitingLinks[i]['source'])=='number'){
      if((link['source']==exisitingLinks[i]['source'] && link['target']==exisitingLinks[i]['target']) || (link['target']==exisitingLinks[i]['source'] && link['source']==exisitingLinks[i]['target'])){
        return 1;
      }
    }else{
      if((link['source']==companyExistsInNodes(exisitingLinks[i]['source']['name']) && link['target']==companyExistsInNodes(exisitingLinks[i]['target']['name'])) || (link['target']==companyExistsInNodes(exisitingLinks[i]['source']['name']) && link['source']==companyExistsInNodes(exisitingLinks[i]['target']['name']))){
        return 1;
      }
    }
  }
  ...
  if(view==1){
    ...
    for(var i=0;i<globalLinks.length;i++){
      if(typeof(globalLinks[i]['source'])=='number'){
        globalLinks[i]['source']=getCompanyAtIndex(globalLinks[i]['source']);
        globalLinks[i]['target']=getCompanyAtIndex(globalLinks[i]['target']);
        //globalLinks[i]['source']=companyExistsInNodes(globalLinks[i]['source']['name']);
        //globalLinks[i]['target']=companyExistsInNodes(globalLinks[i]['target']['name']);
      }
    }
    drawPathView();    
  }else if(view==2){
...
function filterDataAcrossViews(fromview){
   ...
   var endYear = parseInt($("#endyear").val());
    ...
    for(var i=0;i<globalLinks.length;i++){
      var company1Index = globalLinks[i]['source'];
      var company2Index = globalLinks[i]['target'];      
      var company1 = getCompanyAtIndex(company1Index); 
      ...
      if(linkTerminatedYear==-1){
        ...
        for(var i=0;i<globalLinks.length;i++){
      var company1 = globalLinks[i]['source'];
      ...
      if(linkEffYear!=-1){
            if(linkEffYear<effYear){              
              addLinkToPathView2(globalLinks[i],relationshipTypesToShow,showJV,showCB);
            }else if(linkEffYear==effYear){
              if(linkEffYear==endYear){
                if(linkEffQuarter<=endQuarter && linkEffQuarter>=effQuarter){
                  addLinkToPathView2(globalLinks[i],relationshipTypesToShow,showJV,showCB);
    ...
    for(var i=0;i<currentScatternetLinks.length;i++){
      ...
      var company1Index = currentScatternetLinks[i]['source'];
      var company2Index = currentScatternetLinks[i]['target'];
      var company1 = getCompanyAtIndex(company1Index);
      var company2 = getCompanyAtIndex(company2Index);
      var company1Name = company1['name'];
      var company2Name = company2['name'];
          ...
          if(linkEffYear!=-1){
            if(linkEffYear<effYear){                            
              ...
              if(linkEffYear<endYear){
                ...
                if(linkEffQuarter<=endQuarter){
                  if(company1Name=="microsoft corp" || company2Name=="microsoft corp"){
                    console.log("here")
                  }
                  addLinkToScatternet(currentScatternetLinks[i],relationshipTypesToShow,showJV,showCB);    
                }
...
//    console.log(filteredScatterNetLinks);
    //end of scatternet data filtering
    }
...
/*
  console.log("pathview filtered:")
  console.log(filteredPathViewNodes)
  console.log(filteredPathViewLinks)
  console.log("scatternet filtered:")
  console.log(filteredScatterNetNodes)
  console.log(filteredScatterNetLinks)
  console.log("segmenview filtered:")
...
function addLinkToPathView2(currentLink,relationshipTypesToShow,showJV,showCB){
      ...
      var company1 = currentLink['source'];
      var company2 = currentLink['target'];
      var company1Name = currentLink['source']['name'];
      var company2Name = currentLink['target']['name'];
      allTypesInCurrentEdge = getAllAllianceTypesBetween(company1Name,company2Name);
      for(var j=0;j<relationshipTypesToShow.length;j++){
                if(objectIsInList(relationshipTypesToShow[j],allTypesInCurrentEdge)==1){
                  if(objectIsInList(company1,filteredPathViewNodes)==0){
                    filteredPathViewNodes.push(company1);
                  }
                  if(objectIsInList(company2,filteredPathViewNodes)==0){
                    filteredPathViewNodes.push(company2);
                  }
            ...
            if(showCB==1){
              if(edgeHasCrossBorderAlliance(company1Name,company2Name)==1){
                if(objectIsInList(company1,filteredPathViewNodes)==0){
                  filteredPathViewNodes.push(company1);
                }
                if(objectIsInList(company2,filteredPathViewNodes)==0){
            ...
            if(showJV==1){
              if(edgeHasJointVenture(company1Name,company2Name)==1){
                if(objectIsInList(company1,filteredPathViewNodes)==0){
                  filteredPathViewNodes.push(company1);
                }
                if(objectIsInList(company2,filteredPathViewNodes)==0){
...
function addLinkToPathView(currentLink,relationshipTypesToShow,showJV,showCB){
      var allTypesInCurrentEdge,flag=0;
      //console.log("from inside addLinkToPathView");
      //console.log(currentLink);
      var company1 = getCompanyAtIndex(currentLink['source']);
      var company1Name = company1['name'];
      var company2 = getCompanyAtIndex(currentLink['target']);
      var company2Name = company2['name'];
...
*/
function linkIsInFilteredLinks(company1,company2,links){
  for(var i=0;i<links.length;i++){
    if((globalNodes[links[i]['source']]['name']==company1 && globalNodes[links[i]['target']]['name']==company2) || (globalNodes[links[i]['source']]['name']==company2 && globalNodes[links[i]['target']]['name']==company1)){
      return 1;
    }
  }return 0;
}
...
var sourceCompany=getCompanyAtIndex(currentLink['source']);
var targetCompany=getCompanyAtIndex(currentLink['target']);
//console.log("from inside addLinkToSegmentView");
//console.log(sourceCompany);
...
nodes.on("mouseover",function(d){
    ...
    .style("opacity", function(i) {
               if(i.name==d.name || getPartners(d.name).indexOf(i.name) > -1){
                 if(getPartners(d.name).indexOf(i.name) > -1){
                  if(linkIsInFilteredLinks(i.name,d.name,filteredSegmentViewLinks)==1){
                   return 0.9;                
                  }else{
                    return 0.05; 
                  }
